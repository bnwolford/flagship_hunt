...
phenocols <- c("C3_CANCER", "K11_APPENDACUT", "J10_ASTHMA", "I9_AF","I9_CHD", "C3_COLORECTAL", "G6_EPLEPSY", "GOUT", "COX_ARTHROSIS", "KNEE_ARTHROSIS", "F5_DEPRESSIO", "C3_MELANOMA_SKIN", "T1D", "T2D", "ILD", "C3_BRONCHUS_LUNG", "RHEUMA_SEROPOS_OTH")
prscols <- c("AllCancers", "Appendicitis", "Asthma", "Atrial_Fibrillation", "CHD", "Colorectal_Cancer", "Epilepsy","Gout", "Hip_Osteoarthritis", "Knee_Osteoarthritis","MDD", "Melanoma", "T1D","T2D", "ILD", "Lung_Cancer", "Rheumatoid_Arthritis")
#custom_covar and covariates comes from wrapper script
...
for(i in 1:length(phenocols)){
  ...
  pheno <- fread(input=pheno_file, select=c("ID","DATE_OF_BIRTH","PC1","PC2","PC3","PC4","PC5","PC6","PC7","PC8","PC9","PC10",custom_covar,phenocols[i],paste0(phenocols[i],"_DATE"),"END_OF_FOLLOWUP","SEX"), data.table=FALSE)
  ...
  for(k in c("male","female")){
    ...
    for(j in c(1,2,3,4)){
      ...
      result <- c(phenotype, prs, k, minage, maxage, medianAAO, controls, cases, betas, std_errs, pvals, OR, CIpos, CIneg)
...
names(results)<-c("phenotype", "prs", "k", "minage", "maxage", "medianAAO", "controls", "cases", "betas", "std_errs", "pvals", "HR", "CIpos", "CIneg")
write.csv(results, paste0(output_dir,"HRperSD_AgeandSexStratified_",biobank_name,".csv"),row.names=FALSE)
