...
for(i in 1:length(phenocols)){
  for(p in percentiles){
    ...
    pheno <- fread(input=pheno_file, select=c("ID","SEX","DATE_OF_BIRTH","PC1","PC2","PC3","PC4","PC5","PC6","PC7","PC8","PC9","PC10",custom_covar,phenocols[i],paste0(phenocols[i],"_DATE"),"END_OF_FOLLOWUP"), data.table=FALSE)
   ...
   # pheno <- subset(pheno, ANCESTRY=='EUR')
    ...
    survival <- coxph(as.formula(paste0("Surv(AGE,",phenocols[i],") ~ ",prscols[i],"_group +",covariates)), data=males, na.action=na.exclude)
    ...
    #Perform survival analysis
    survival <- coxph(as.formula(paste0("Surv(AGE,",phenocols[i],") ~ ",prscols[i],"_group + ",covariates)), data=females, na.action=na.exclude)
...
femaleresults<-data.frame(femaleresults)
names(femaleresults)<-c("phenotype", "prs", "groups", "controls", "cases", "betas", "std_errs", "pvals", "HR", "CIpos", "CIneg")
maleresults<-data.frame(maleresults)
names(maleresults)<-c("phenotype", "prs", "groups", "controls", "cases", "betas", "std_errs", "pvals", "HR", "CIpos", "CIneg")
write.csv(maleresults, paste0(output_dir,"HR_MaleSample_",biobank_name,".csv"))
write.csv(femaleresults, paste0(output_dir,"HR_FemaleSample_",biobank_name,".csv"))
