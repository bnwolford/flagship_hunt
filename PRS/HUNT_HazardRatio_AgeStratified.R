  ...
  for(p in percentiles){
    ...
    for(j in c(1,2,3,4)){
      ...
      if(p[2] == 0.01){
        ...
        CIpos <- exp(betas+1.96*std_errs)
        CIneg <- exp(betas-1.96*std_errs)
        result <- matrix(c(phenotype, prs, minage, maxage, medianAAO, groups, controls[c(1,length(controls))], cases[c(1,length(cases))], betas, std_errs, pvals, OR, CIpos, CIneg), nrow=2, ncol=14)
        results <- rbind(results, result)
      }
